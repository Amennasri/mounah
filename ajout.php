...
$nomError = $descriptionError = $photoError = $locationError = $expiryDateError = "";
$nom = $description = $photo = $expiry_date = $latitude = $longitude = "";
$isUpdate = false;

// Si un ID est passé, récupérer les données existantes pour mise à jour
if ($donation_id) {
    $isUpdate = true;
    $stmt = $db->prepare("SELECT nom, description, photo, expiry_date, ST_X(location) AS latitude, ST_Y(location) AS longitude 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($photo) && !$newPhoto) {
        $photoError = 'Veuillez sélectionner une image.';
        $isSuccess = false;
    } else if ($newPhoto) {
        ...
        if ($_FILES["photo"]["size"] > 500000) {
            $photoError = "La taille du fichier ne doit pas dépasser 500KB.";
            $isUploadSuccess = false;
        }
        if ($isUploadSuccess && !move_uploaded_file($_FILES["photo"]["tmp_name"], $photoPath)) {
    ...
    if ($isSuccess) {
        ...
        if ($isUpdate) {
            $stmt = $db->prepare("UPDATE donations 
                                  SET nom = ?, description = ?, photo = ?, expiry_date = ?, location = ST_GeomFromText(?) 
                                  WHERE id = ? AND user_id = ?");
            $stmt->execute([$nom, $description, $photoPath, $expiry_date, $location, $donation_id, $id_user]);
        } else {
            $stmt = $db->prepare("INSERT INTO donations (user_id, nom, description, photo, expiry_date, location) 
                                  VALUES (?, ?, ?, ?, ?, ST_GeomFromText(?))");
            $stmt->execute([$id_user, $nom, $description, $photoPath, $expiry_date, $location]);
        }
        header("Location: index.php");
        exit();
    }
